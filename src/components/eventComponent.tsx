...
export default function EventComponent({
  ...
  useEffect(() => {
    if (event.ticketInfo && event.ticketInfo.length > 0) {
      ...
      const sortedTickets = [...event.ticketInfo].sort(
        (a, b) => a.price - b.price
      );
  ...
  return (
    <Link
      ...
      className={`rounded-lg bg-white bg-opacity-10 backdrop-blur-sm ${className}`}
    >
      ...
      <h3
        className={
          "text-xl font-medium mt-2 mb-1 capitalize mix-blend-difference"
        }
      >
        {event.eventName}
      </h3>
      <div className={"flex justify-between items-center"}>
        <div className={""}>
          ...
          <h4 className={"text-gray-500 text-wrap font-medium mt-1"}>
            {event.venue.name} - {event.venue.street},{event.venue.city}
          </h4>
        </div>
        <h4 className={"text-primary"}>
          {event.currency} {lowestPrice === 0 ? "Free" : lowestPrice.toFixed(2)}
        </h4>
      </div>
    </Link>
  );
