...
import { fetchExchangeRates, fetchPawaPayConfigs } from "./cartSlice";
...
const setAuthCookies = (token: string, user: any) => {
    ...
    setCookie(null, 'user', JSON.stringify(user), { maxAge: 7 * 24 * 60 * 60, path: '/' });
};
...
export const signInHost = createAsyncThunk<any, { email: string; password: string }>(
    'auth/signIn',
    async ({ email, password }, { rejectWithValue }) => {
        try {
            ...
            return rejectWithValue({ success: false, status: e.response?.status, message: e.response?.data?.msg });
        }
    }
);
